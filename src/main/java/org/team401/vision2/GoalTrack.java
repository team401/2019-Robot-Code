package org.team401.vision2;

import org.snakeskin.hardware.Hardware;
import org.team401.robot2019.vision2.VisionConstants;
...
public class GoalTrack {
    ...
    public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
        ...
        if (distance < VisionConstants.maxTrackerDistance) {
    ...
    synchronized void pruneByTime() {
        double delete_before = Hardware.INSTANCE.getRelativeTime() - VisionConstants.maxGoalTrackAge;
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            double t_now = Hardware.INSTANCE.getRelativeTime();
            int num_samples = 0;
            for (Map.Entry<Double, Pose2d> entry : mObservedPositions.entrySet()) {
                if (t_now - entry.getKey() > VisionConstants.maxGoalTrackSmoothingTime) {
                    continue;
                }
                ++num_samples;
    ...
    public synchronized double getStability() {
        return Math.min(1.0, mObservedPositions.size() / (VisionConstants.cameraFrameRate * VisionConstants.maxGoalTrackAge));
    }
